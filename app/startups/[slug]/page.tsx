...
import { getPeopleByIds, getResearchAreas, getStartup, getStartups, getTrlScale } from "@/lib/data";
...
export default async function StartupPage({ params }: { params: Promise<{ slug: string }> }) {
  const s = await getStartup((await params).slug);
  if (!s) notFound();
  const [areas, scale] = await Promise.all([getResearchAreas(), getTrlScale()]);
    ...
    <main id="main">
      ...
      <div className="container-x section-y !pt-12">
        <div className="mx-auto max-w-5xl">
          ...
          <MoreLink href="/startups" className="mt-10">All ventures</MoreLink>
        </div>
      </div>
    </main>
  );
