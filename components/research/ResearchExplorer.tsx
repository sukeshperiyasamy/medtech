      ...
      {faculty.map((f) => (
        <li key={f.id} className="flex items-center gap-3">
          {f.photo ? (
            <Image
              src={f.photo.src}
              unoptimized={isRemote(f.photo.src)}
              alt=""
              width={40}
              height={40}
              ...
              >
                <div className="relative overflow-hidden border-b border-line bg-paper bg-grid px-8 pb-8 pt-7">
                  ...
                  <ul className="mt-6 flex flex-wrap gap-2">
                    {a.keywords.map((k) => (
                      <li key={k} className="rounded-xs border border-line bg-white px-2.5 py-1 font-mono text-[0.7rem] uppercase tracking-[0.06em] text-ink-2">
                        {k}
                      </li>
                    ))}
                  </ul>
          ...
          {areas.map((area, i) => {
            ...
            return (
              <li key={area.id} className="border-b border-line">
                ...
                >
                  <span className={cn("font-mono text-xs transition-colors", isActive ? "text-blue" : "text-muted")}>
                    {pad2(i + 1)}
                  </span>
            ...
            <li key={area.id} className="border-b border-line">
              <h3>
                ...
                >
                  ...
                  <span className="flex-1 text-[1.2rem] leading-tight tracking-[-0.015em] text-ink">{area.title}</span>
                  <Plus aria-hidden className={cn("size-5 text-muted transition-transform duration-300", open && "rotate-45")} />
                </button>
              </h3>
