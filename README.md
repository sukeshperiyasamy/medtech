...
public/
  images/
    brand/                  Official IIT Jodhpur logo and AIIMS Jodhpur seal (unmodified)
    backgrounds/            Decorative section backgrounds (homepage hero)
    centre/                 Centre building photograph
    people/                 Locally hosted portraits (faculty portraits otherwise load from iitj.ac.in)
    gallery/                The single gallery collection (generated by build-gallery.py)
    achievements/           Official achievement posters and photographs
    icmi-2025/              Graded ICMI 2025 photo set (News, Medical Technologies)
  icons/                    Home-screen / web-app manifest icons
  videos/                   Walkthrough video and its poster
...
4. Keep large originals out of git. Put them under `medtech-media/source/` and add the folder to `.gitignore`, as was done for `medtech-media/source/icmi-2025-heic/`.

`next/image` then serves AVIF/WebP at the right size for each device.

### Media library

Originals live in `medtech-media/`:

- **`img/`** — event photo archive, source of the gallery (tracked).
- **`source/`** — other originals: `brand/` (logo and seal masters), `people/`, `centre/`, `achievements/` (the achievements deck), `backgrounds/` (hero background master) and `icmi-2025-heic/` (HEIC masters, ignored by git for size).
- **`vedio/`** — video master (ignored by git: over GitHub's 100 MB limit).
...
- The IIT Jodhpur logo and AIIMS Jodhpur seal in `public/images/brand/` are official assets. Use them unmodified and at equal size. Their originals are in `medtech-media/source/brand/`.
